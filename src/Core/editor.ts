...
implements EditorInterface<T> {
    ...
    getSelectionData (
        ...
        if (card.length > 0) {
            const component = this.card.find(card)
            if (component && component.getSelectionNodes) {
            ...
            } else if (!component?.isEditable) return
        }
        ...
        contents.querySelectorAll('li').forEach((child) => {
            ...
            if (!dataId) return
            const currentElement = this.container
                .get<HTMLElement>()
            ...
            if (
                ...
                childElement.prepend(
                    node.clone($(currentElement.firstChild), true, false)
                )
            }
