...
class List implements ListModelInterface {
    ...
    isEmptyItem (node: NodeInterface): boolean {
        ...
        return (
            ...
            (nodeApi.isEmpty(node) ||
                ...
                (children.length === 1 ? (nodeApi.isCustomize(node) && children[0].isCard()) || children[0].name === 'br' : children.length === 2 && nodeApi.isCustomize(node) && !!children[0].isCard() && children[1].name === 'br'))
        )
    }
        ...
        blocks.forEach((block) => {
            ...
            if (block.name === 'li') {
                ...
                Object.keys(attributes).forEach((name) => {
                    if (name !== DATA_ID && name !== 'id' && (globals as any)[name]) {
                ...
                Object.keys(styles).forEach((name) => {
                    if (!(globalStyles as any)[name]) styles[name] = ''
                })
                toBlock.css(styles)
    ...
    insert (fragment: DocumentFragment, range?: RangeInterface) {
        ...
        if (fragment.childNodes.length === 0) {
            ...
            if (node.isEmpty(startLi) || this.isEmptyItem(startLi)) {
                ...
                // 把光标位置放到前面的li里面
                    prevElement.append(startLi.children())
                }
    ...
    blockToItem (
        ...
        Object.keys(styles).forEach((name) => {
            if (!(globalStyles as any)[name]) delete styles[name]
        })
        item.css(styles)
        // 替换
    ...
    isFirst (range: RangeInterface) {
        ...
        // 找到li节点
        const node =
            startNode.name === 'li' ? startNode : startNode.closest('li')
        // 如果没有li节点
        ...
        // 如果选区中只有一个节点，并且是br标签
        if (contents.childNodes.length === 1 && firstChild.name === 'br') {
            return true
        }
