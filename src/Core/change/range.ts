    ...
    private setCardRang (range: RangeInterface) {
        ...
        if (component) {
            ...
            if (
                ...
                const comparePoint = () => {
                    const docRang = Range.create(this.engine)
                    docRang.select(cardCenter, true)
                    return docRang.comparePoint(startNode, startOffset) < 0
                }
        ...
        if (range.collapsed) {
            ...
            if (
                ((startNode.isElement() &&
                    ...
                    (startOffset === 2 &&
                        startNode.get<Node>()?.childNodes.length === 2 &&
