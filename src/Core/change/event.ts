...
import { isCard, isEditable } from '../node/utils'
...
class ChangeEvent implements ChangeEventInterface {
    ...
    private readonly options: ChangeEventOptions;
    ...
    onSelect (
        ...
        this.onContainer('keydown', () => {
            this.keydownRange = Range.from(this.engine)
        })
        // 补齐通过键盘选中的情况
        ...
        this.onContainer('dragover', (e: DragEvent) => {
            ...
            if (cursor.length !== 0) {
                ...
                cursor.css({
                    ...
                    top: Math.round(window.scrollY + (rect.y || 0)) + 'px',
                    left: Math.round(window.scrollX + (rect.x || 0)) + 'px'
                })
