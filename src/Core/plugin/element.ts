    ...
    schema (): SchemaRule | SchemaGlobal | Array<SchemaRule> {
        ...
        if (this.attributes) {
            ...
            Object.keys(this.attributes).forEach((attributesName) => {
                ...
                (attributes as any)[attributesName] = attributesValue
                attributesValue
                    .match(/@var\d/g)
                    ?.forEach((regMatch: string) => {
                        if (!this.variable) {
                            throw new Error('Please specify the variable type')
                        }
                        (attributes as any)[attributesName] = this.variable[regMatch]
                    })
            })
        }
        if (this.style) {
