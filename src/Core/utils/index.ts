...
export const createMarkdownIt = (
    editor: EditorInterface,
    presetName: MarkdownIt.PresetName = 'default'
) => {
    const markdown = new MarkdownIt(presetName, {
        html: true,
        typographer: true,
        linkify: true
    })
...
export const convertMarkdown = (
    ...
    tokens.forEach((token, index) => {
        ...
        if (type === 'inline' && children) {
            ...
            if (checkInline && children.find((child: any) => child.type === 'image' || child.type.endsWith('_inline') || child.type.endsWith('_open'))) {
                isHit = true
            }
