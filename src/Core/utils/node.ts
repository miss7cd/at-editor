...
export const getDocument = (node?: Node): Document => {
    if (
        typeof document === 'undefined' && typeof (global as any).__amWindow === 'undefined'
    ) {
        throw new Error('document is not defined,If you are using ssr, you can assign a value to the `__amWindow` global variable.')
    }

    return node
        ? node.ownerDocument || (node as any).document || node
        : typeof document === 'undefined'
            ? (global as any).__amWindow.document
            : document
}
