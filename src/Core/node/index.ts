...
class NodeModel implements NodeModelInterface {
    ...
    isNestedBlock (node: NodeInterface | Node) {
        ...
        const element = ((node as NodeInterface).length ? (node as any)[0] : node) as Element
    ...
    isEmptyWidthChild (node: NodeInterface) {
        ...
        for (let i = 0; i < childNodes.length; i++) {
            ...
            if (child.nodeType === Node.TEXT_NODE) {
                if ((child as any).data.replace(/\u200b/g, '') !== '') return false
            ...
            if (source.name === outer.name) {
                ...
                Object.keys(attributes).forEach((key) => {
                    if (!outer.attributes(key)) {
                        ...
                        if (
                            (attributes as any)[key] !== undefined &&
                            attributes.indexOf((attributes as any)[key]) < 0
                        ) {
                            attributes.push((attributes as any)[key])
                        }
    ...
    insert (
        ...
        if (isNodeEntry(node)) {
            if (node.length === 0) throw new Error('Not found node')
            node = node[0]
        }
        const editor = this.editor
        if (!isEngine(editor)) return
