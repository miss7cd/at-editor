...
class NodeEntry implements NodeInterface {
    ...
    isRoot (root?: Node | NodeInterface) {
        ...
        return (
            ...
            isRoot(element, root ? (root as any)[0] ?? root : undefined)
        )
    }
    ...
    inEditor (root?: Node | NodeInterface) {
        ...
        return (
            !!element && inEditor(element, root ? (root as any)[0] ?? root : undefined)
        )
    }
    ...
    css (
        ...
        if (val === undefined) {
            ...
            if (!element) return ''
            return (
                element.style[(toCamelCase as any)(key)] ||
        ...
        this.each((node) => {
            ...
            element.style[(toCamelCase as any)(key)] = val.toString()
    ...
    traverse (
        ...
        callback(this as NodeInterface)
        walk(this)
    }
    ...
    findParent (
        ...
        const element = ((container as any)[0] ?? container) as Node
        let parent = element.parentElement ?? element.parentNode
        if (this.length === 0 || !parent) return null
        let node: Node = this[0]
        while ((parent = node.parentElement ?? node.parentNode) && parent !== element) {
            node = parent
        }
