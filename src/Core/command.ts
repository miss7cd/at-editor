...
class Command implements CommandInterface {
    private readonly editor: EditorInterface;
    ...
    queryEnabled (name: string) {
        ...
        // 当前激活非可编辑卡片时全部禁用
        if (cardApi.active) {
            if ((isMarkPlugin(plugin) || plugin.kind === 'plugin') && cardApi.active.executeMark) {
                return true
            }
    ...
    executeMethod (name: string, method: string, ...args: any) {
        const editor = this.editor
        const plugin = editor.plugin.components[name]
        if (plugin && (plugin as any)[method]) {
            try {
                const result = (plugin as any)[method](...args)
                return result
            } catch (error: any) {
