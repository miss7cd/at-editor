...
class Mark implements MarkModelInterface {
    ...
    compare (
        ...
        const compareClass = (classNames: Array<string>): string => {
            for (let i = 0; i < schemas.length; i++) {
                ...
                if (schemaRule.attributes && 'class' in schemaRule.attributes && schemaRule.attributes.class && schema.checkValue(schemaRule.attributes, 'class', classNames.join(' ').trim())) {
        ...
        // 样式名称不一致
        if (!sourceClasses.every((sourceClass) => targetClasses.indexOf(sourceClass) !== -1)) {
            return false
        }
        return true
    }
    ...
    contain (source: NodeInterface, target: NodeInterface) {
        ...
        return (
            ...
            Object.keys(styles).every((key) => !(sourceStyles as any)[key])
        )
    }
        ...
        if (
            ...
            if (rightNodes.filter((child) => !child.isCursor()).length > 0) {
                ...
                // 右侧有文本
                    ...
                    if (removeMark && removeMark.length > 0 && markParent && node.isMark(markParent)) {
                        while (true) {
                ...
                const handleAt = (node: Node | null, findPrev: boolean) => {
                    ...
                    while (node) {
                        ...
                        if (node.textContent === atText) {
                            ...
                            if ((alignNode && nodeApi.isInline(alignNode)) || (!alignNode && parent && nodeApi.isInline(parent))) {
                                break
                            }
    ...
    splitOnExpanded (
        ...
        if (
            !(
                (cardStart.length > 0 &&
                    cardStart.attributes(CARD_TYPE_KEY) === 'inline') ||
                (cardEnd.length > 0 &&
                    cardEnd.attributes(CARD_TYPE_KEY) === 'inline')
            )
        ) {
            // 开始非mark标签父节点
    ...
    wrapByNode (
        ...
        if (nodeApi.isMark(node)) {
            ...
            // eslint-disable-next-line no-unmodified-loop-condition
            while (nodeApi.isMark(targetNode) && targetChildrens.filter((child) => !child.isCursor()).length === 1 && plugin && curPlugin && plugin.mergeLeval <= curPlugin.mergeLeval) {
            ...
            let isHandle = false
            // eslint-disable-next-line no-unmodified-loop-condition
            while (targetNode && nodeApi.isMark(targetNode) && plugin && curPlugin && plugin.mergeLeval > curPlugin.mergeLeval) {
                isHandle = true
            ...
            commonAncestorNode.traverse(
                (child) => {
                    ...
                    if (isEditable || (!selection?.anchor || !child.equal(selection.anchor))) {
                        if (started) {
                            if (!isEditable && selection?.focus && child.equal(selection.focus)) {
                                started = false
                                return false
                            }
