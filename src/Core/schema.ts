...
import {    NodeInterface,SchemaAttributes,    SchemaBlock,    SchemaGlobal,    SchemaInterface,    SchemaRule,    SchemaStyle,    SchemaValue,    SchemaValueObject} from '../types'
...
class Schema implements SchemaInterface {
    ...
    add (
                        ...
                        if (key === 'style') {
                            Object.keys(rule.attributes!.style).forEach(
                                (styleName) => {
                                    ...
                                    // @ts-ignore
                                    if (this.data.globals[rule.type][key] && this.data.globals[rule.type][key][styleName] === rule.attributes!.style[styleName]) {
                                        // eslint-disable-next-line @typescript-eslint/ban-ts-comment
                                        // @ts-ignore
        ...
        const getCount = (rule: SchemaRule) => {
            ...
            Object.keys(aAttributes).forEach((attributesName) => {
                const attributesValue = (aAttributes as any)[attributesName]
                if (
            ...
            Object.keys(aStyles).forEach((stylesName) => {
                const stylesValue = (aStyles as any)[stylesName]
    ...
    find (callback: (rule: SchemaRule) => boolean): Array<SchemaRule> {
        ...
        SCHEMA_KEYS.forEach((key) => {
            if (key !== 'globals') {
                const rules = ((this.data as any)[key] as Array<SchemaRule>).filter(
                    callback
                )
                schemas.push(...rules)
            }
    ...
    checkValue (
        ...
        if (isSchemaValueObject(rule)) {
            ...
            rule = rule.value
        } else if (!force || attributesValue === undefined) {
        // 默认都不为强制的
            return true
        }
        /**
    ...
    isAllowIn (source: string, target: string) {
        ...
        return this.data.blocks.some((block) => {
            if (block.name !== target) return false
            const schema = block as SchemaBlock
            return !!(schema.allowIn && schema.allowIn.indexOf(source) > -1)
        })
    }
