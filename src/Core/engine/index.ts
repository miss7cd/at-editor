...
import { ContainerInterface, EngineInterface, EngineOptions } from '../../types/engine'
...
class Engine<T extends EngineOptions = EngineOptions> extends Editor<T>
     ...
     * @returns
     */
    async getValueAsync (ignoreCursor = false, callback?: (name: string, card?: CardInterface, ...args: any) => boolean | number | void): Promise<string> {
        const asyncLogic = async () => {
            for (const pluginName in this.plugin.components) {
                const plugin = this.plugin.components[pluginName]
                const result = await new Promise((resolve) => {
                    if (plugin.waiting) {
                        plugin.waiting(callback).then(() => resolve(true)).catch(resolve)
                    } else {
                        resolve(true)
                    }
                })
            ...
            return this.getValue(ignoreCursor)
        }
