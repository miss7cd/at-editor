...
const getIndent = (node: Element) => {
    let dataIndent = (node as any)['data-indent']
    if (!dataIndent) {
...
const listToText = (node: Element) => {
    ...
    const classz = (node as any).class
...
const elementToText = (node: Element, intoCard = false) => {
    ...
    if ((node as any)[CARD_KEY] === 'checkbox') {
        const value = decodeCardValue((node as any)[CARD_VALUE_KEY])
        const checked = value.checked
        return checked ? '✅' : '🔲'
    }
    if (intoCard || !(node as any)[CARD_KEY] || (node as any)[CARD_EDITABLE_KEY] === 'true') {
