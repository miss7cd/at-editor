import { DATA_ELEMENT, CURSOR, ANCHOR, FOCUS, DATA_TRANSIENT_ELEMENT, UI, ROOT, CARD_LOADING_KEY, CARD_SELECTOR, UI_SELECTOR, DATA_TRANSIENT_ATTRIBUTES } from '../constants'
...
export function isPlainObject (o: unknown): o is InspectableObject {
    ...
    if (!(prot as any).hasOwnProperty('isPrototypeOf')) {
        return false
    }
    ...
    if (element.nodeType === Node.ELEMENT_NODE) {
        ...
        if (transientElements) {
            ...
            const element = transientElements.find(
                (element) => element === (node as any)[0]
            )
            if (element) {
                if ((element as any).__card_root) {
                    loadingCards?.push((element as any).__card_root)
                }
                return true
            }
        }
