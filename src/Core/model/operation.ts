...
import { DIFF_DELETE, DIFF_EQUAL, DIFF_INSERT, diff_match_patch as DiffMatchPatch, patch_obj as PatchObj } from 'diff-match-patch'
import { CARD_KEY, CARD_LOADING_KEY, CARD_TYPE_KEY, CARD_VALUE_KEY, DATA_ID } from '../constants'
...
const transform = (engine: EngineInterface, records: MutationRecord[]) => {
    ...
    const expandSibling = (
        ...
        const { previousSibling: currentPreviousSibling, nextSibling: currentNextSibling } = mutationNode
        if ((previousSibling && (!currentPreviousSibling || currentPreviousSibling.compareDocumentPosition(previousSibling) & globalThis.Node.DOCUMENT_POSITION_PRECEDING)) || currentPreviousSibling?.parentNode !== mutationNode.node) {
        ...
        if ((nextSibling && (!currentNextSibling || currentNextSibling.compareDocumentPosition(nextSibling) & globalThis.Node.DOCUMENT_POSITION_FOLLOWING)) || currentNextSibling?.parentNode !== mutationNode.node) {
        ...
        if (mutationNode.previousSibling !== previousSibling && mutationNode.previousSibling?.contains(previousSibling)) {
            mutationNode.previousSibling = mutationNode.previousSibling.previousSibling
        }
        if (mutationNode.nextSibling !== nextSibling && mutationNode.nextSibling?.contains(nextSibling)) {
            mutationNode.nextSibling = mutationNode.nextSibling.nextSibling
        }
    }

    const validMutationNode = (node: DOMNode, previousSibling: DOMNode | null, nextSibling: DOMNode | null) => {
        ...
        for (const mutationNode of mutationNodes) {
            ...
            if (node === currentNode || currentNode.contains(node)) {
                ...
                const { previousSibling: oldPreviousSibling, nextSibling: oldNextSibling } = mutationNode
...
const diff = (engine: EngineInterface, a: BaseNode[], b: BaseNode[], path: Path, index = 0, isLoading = false) => {
...
const transformText = (a: string, b: string, path: Path, isLoading = false) => {
    const operations: Operation[] = []
    const patches = new DiffMatchPatch().patch_make(a, b)
    Object.keys(patches).forEach((key) => {
        const patch: PatchObj = (patches as any)[key] as any
...
const isLoadingCard = (engine: EngineInterface, element: Element) => {
    const cardKey = (element as any)[CARD_KEY]
    const cardType = (element as any)[CARD_TYPE_KEY]
    const dataId = (element as any)[DATA_ID]
