...
export const applyToDOM = (engine: EngineInterface, operation: Operation, isRemote: boolean) => {
    const { card } = engine
    const { path } = operation
    const { parent, node: domNode, offset: domOffset } = findDOMByPath(engine, engine.container[0], path)
