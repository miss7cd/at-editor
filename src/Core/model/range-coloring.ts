...
class RangeColoring {
    ...
    drawBackground (
        ...
        let child: NodeInterface | undefined = BACKGROUND_TO_ELEMNT.get(uuid)
        ...
        const clear = () => {
            if (child && child.length > 0) {
                ...
                const canvas = (child[0] as any).__canvas
    ...
    drawCursor (selector: RangeInterface | NodeInterface, member: CollaborationMember, showInfo?: boolean) {
    ...
    updateBackgroundPosition () {
        for (const node of BACKGROUND_TO_ELEMNT.values()) {
            ...
            if (!child) continue
            const range = (child as any).__range
    ...
    updateCursorPosition () {
        for (const [key, node] of CURSOR_TO_ELEMNT) {
            ...
            let target = (child as any).__target
            if (!target) {
                node.remove()
                CURSOR_TO_ELEMNT.delete(key)
                return
    ...
    updateCardPosition () {
        ...
        for (const [key, node] of MASK_TO_ELEMNT) {
            const child = node.get<HTMLElement>()
            if (!child || !(child instanceof HTMLElement)) continue
            const target: Element | undefined = (child as any).__node
        ...
        (this.engine.scrollNode?.get<Element>() ?? this.root.get<Element>())
            ...
            .forEach((dataElement) => {
                if (dataElement.classList.contains(USER_MASK_CLASS)) {
                    const target: Node | undefined = (dataElement as any).__node
