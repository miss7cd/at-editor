...
class CardToolbar implements CardToolbarInterface {
    ...
    private createDnd (content: string, title: string) {
        const dndNode = $(
            `<div ${DATA_ELEMENT}="${UI}" class="data-card-dnd" draggable="true" dnd-trigger-key="${(this.card.constructor as CardEntry).cardName}" drag-card-trigger="${this.card.id}" ${DATA_CONTENTEDITABLE_KEY}="false">
