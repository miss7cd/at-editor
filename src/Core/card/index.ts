...
class CardModel implements CardModelInterface {
    ...
    replaceNode (node: NodeInterface, name: string, value?: CardValue) {
        ...
        const cardNode = transformCustomTags(`<card ${dataId ?? `${DATA_ID}="${dataId}"`} type="${type}" name="${name}" value="${encodeCardValue(value)}"></card>`)
        ...
        if (!container.get() || editor.container.equal(container)) {
            ...
            if (card) {
                ...
                if (!isCurrentActiveCard) {
                    if ((card.constructor as CardEntry).singleSelectable !== false && (trigger !== CardActiveTrigger.CLICK || isEngine(editor))) {
                        this.select(card, event)
                    }
                    if (!card.isEditable && (card.constructor as CardEntry).autoSelected !== false) {
    ...
    > (name: string, value?: E, ...args: any) {
        const editor = this.editor
        if (!isEngine(editor)) throw new Error('Engine not found')
    ...
    focusPrevBlock (
        ...
        const editor = this.editor
        if (!isEngine(editor)) throw new Error('Engine not initialized')
        let prevBlock
        if (card.type === 'inline') {
