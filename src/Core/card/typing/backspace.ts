...
class Backspace {
    ...
    trigger (event: KeyboardEvent) {
        ...
        if (!card) {
            ...
            if (!(event as any).isDelete && prevNode && prevNode.isCard() && (!prevParent || !this.engine.node.isCustomize(prevParent))) {
        ...
        if ((event as any).isDelete) return true
        // inline 卡片
        ...
        // 改变了光标选区，再次触发事件
        return this.engine.trigger('keydown:backspace', event) !== false
    }
}
